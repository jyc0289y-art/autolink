...
> **최종 갱신**: 2026-04-30 (P29.1.1.0 핸드오프 시점)
> **세션**: P29.1.1.0
...
- **새 세션 시작 전 사용자 점검 필요**:
  1. Google Drive Connector — 계정 확인 (현재 cjy@hojinri.com 회사 계정 연결, AutoLink는 jyc0289y@gmail.com 필요 또는 무관)
  2. Gmail Connector — 데스크톱 앱에서도 권한·계정 동기화 확인
  3. GitHub Connector — 데스크톱 앱에 추가 (현재 미연결, gh CLI로 우회 중이므로 우선순위 낮음)
- **진단 결과 (P29.1.1.0)**:
  - Drive: 🟢 작동 (회사 계정 cjy@hojinri.com)
  - Gmail: 🔴 도구 미노출 (서버 ID `0c984139-...`)
  - GitHub: 🔴 Connector 미등록 (gh CLI는 정상 작동)
- **새 세션에서 즉시 진행할 작업**: ① 3개 MCP 작동 테스트 → ② 작동 확인되면 Gmail 라벨 5개 자동 생성 → ③ SKILL.md를 API 버전으로 재전환 → ④ ACT-007 ✅ 처리

### [2026-04-29 23:30:00] 운영비 투명성 + 사용자 액션 가이드 + 사업계획서 v1.1
- **무엇을**: 사장님 운영비 부담을 사전 고지하여 분쟁 예방하는 5중 장치 구축
- **왜**: 사용자 지적 — "이상 징후 카톡" 같은 표현은 카카오 알림톡 유료 (건당 9~15원). 사전 고지 없이 청구되면 분쟁. 사장님이 비용 부담을 판단할 수 있어야 함.
- **결과물**:
  - [`business-plan_automation-dashboard-service.md`](../../business-plan_automation-dashboard-service.md) **v1.1**: §3.6 외부 API 비용 투명성 신규, 시그니처 케이스 v1.1 재구성
  - [`docs/20260429_224447_운영비_가이드.md`](../20260429_224447_운영비_가이드.md): 알림 채널·API 비용 매트릭스, 시그니처 케이스별 운영비 추정, 사장님 안내 양식, 자동 차단 로직, 분쟁 시뮬레이션
  - [`docs/20260429_224059_사용자_액션_가이드.md`](../20260429_224059_사용자_액션_가이드.md): ACT-002/006/007/008 단계별 절차
  - [`legal/templates/sow-template.md`](../../legal/templates/sow-template.md): §3.7 외부 API 운영비 신규 (3구간 안내, 의뢰자 직접 결제, 80%/100% 자동 차단)
  - [`site/index.html`](../../site/index.html): #cost-transparency 섹션 신규 (무료 vs 유료 카드), 차별화 #3 "카톡 알림 기본 포함" → "원하는 채널로 알림", 약관 동의에 운영비 항목 추가
  - [`site/style.css`](../../site/style.css): #cost-transparency, .cost-grid, .cost-card 스타일
- **5중 장치 요약**:
  1. 사이트 운영비 투명성 섹션 (무료 vs 유료 분리 표시)
  2. 의뢰 폼 약관 동의에 "유료 API 사장님 직접 결제" 동의 항목 추가
  3. SOW §3.7에 무료/유료/3구간 예상 비용 명세 의무
  4. 자동 차단 로직 (80%/100% 알림 + 자동 중단)
  5. 운영비 가이드 별도 문서 (사장님 안내 양식 포함)
- **다음 단계**: 사용자 결정 — 시그니처 데모 #1 또는 #4 프로토타입

### [2026-04-29 23:00:00] 옵션 A — 우산 메시지 확장
- **무엇을**: 사이트 메시지를 "크롤링+대시보드"에서 "자동화 4영역(엑셀·API·크롤링·대시보드+AI)" 우산으로 확장
- **왜**: KIPRIS 검토 후 BMW 충돌 회피 + "스마트스토어 매출 시트로", "엑셀 자동화" 등 가장 큰 SMB 시장이 현재 메시지로는 잡히지 않아 이탈 우려
- **결과물**:
  - [`site/index.html`](../../site/index.html):
    - Hero 칩 6종으로 확장: 엑셀·시트 / API 연동 / 데이터 수집 / 대시보드 / AI 분석 / 카톡 알림
    - 신규 섹션 `#service-areas`: 4가지 영역 카드 (각 영역에 사장님 말투 예시 3개)
    - Portfolio 6장 재구성: 매출 자동 집계(엑셀·API 신규) + 가격 모니터링 + 월말 보고서(엑셀·PDF 신규) + 리뷰 AI 분석(시그니처) + 매출 통합 대시보드 + 메타
    - 카드별 영역 태그 추가 (엑셀·API / 크롤링 / 엑셀·PDF / 크롤링·AI / API·대시보드 / 메타)
    - 차별화 #1 "엑셀이 아닌 사이트로" → "원하는 형태로 결과물" (엑셀·시트·웹·PDF·카톡 모두 OK)
  - [`site/style.css`](../../site/style.css): `#service-areas`, `.areas-grid`, `.area-card`, `.area-num`, `.area-examples`, `.card-tags`, `.card-tag-area` 신규 스타일
- **다음 단계**: 옵션 C 단계 진입 (페이지 A 운영 후 페이지 B 별도 등록) — 첫 의뢰 수주 후

### [2026-04-29 22:30:00] 계약 보호 Step 2 + Step 3 완료
- **Step 2 — SKILL.md 3-게이트 재설계**: `~/.claude/scheduled-tasks/autolink-intake/SKILL.md`
  - 자동 응답 ≠ 자동 확정 원칙 명시 ("본 SKILL은 게이트 1까지만 자동화")
  - 자연어 → 6변수 자동 매핑 (각 점수 옆에 추론 근거 인용)
  - 위험 플래그 8종 자동 감지 (약관/법령/모호/일정/D등급/외부 시스템/동의 누락)
  - 진영 대표 검토 체크리스트 8항목
  - 응대 초안 3종 (A: 정식 견적+SOW / B: 추가 정보 / C: 정중 거절)
  - 사장님에게 견적 자동 발송 절대 금지 명시
- **Step 3 — 법무 템플릿 작성**: `legal/templates/`
  - [`quote-template.md`](../../legal/templates/quote-template.md): 비계약 견적, 14일 유효, ±30% 조정
  - [`sow-template.md`](../../legal/templates/sow-template.md): 계약 본체. 포함/제외 양쪽 명시, 객관적 인수 기준, 위약 조항
  - [`terms-of-service.md`](../../legal/templates/terms-of-service.md): 14조 약관. 책임 한도(거래 총액), 간접손해 면책, 불가항력, 분쟁 관할(서울중앙지법). ⚠️ 변호사 검토 권장
  - [`_README.md`](../../legal/templates/_README.md): 사용 흐름·변호사 검토 트리거
- **다음 단계**: 변리사·변호사 상담 (ACT-006). B등급 첫 수주 전 필수.
